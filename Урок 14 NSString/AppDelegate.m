...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
 ...
 /*
    ...
    for (NSString* string in array) {
        ...
        if ([string isEqualToString:@"string1"]) { // Сравниваем строки
...
/*
    ...
    if (range.location != NSNotFound) { // Если не нашел то
        NSLog(@"range = %@", NSStringFromRange(range));
    } else {
        NSLog(@"Not Found");
    }
    
    NSString* string1 = @"Hello World World";
    
    NSLog(@"%@", string1);
    
    NSRange range1 = [string1 rangeOfString:@"WorlD" options:NSCaseInsensitiveSearch | NSBackwardsSearch]; // В независимоссти от кейс! | С конца предложения!!! Посмотреть подробнее в хедере !
    
    if (range1.location != NSNotFound) { // Если не нашел то
...
*/
  ...
  //  text = [text substringToIndex:10]; // До десятого индекса
    
    //text = [text substringFromIndex:10]; // После 10 индекса
    
   // text = [text substringWithRange:NSMakeRange(10, 10)]; // Рендж
    
/*
    // Поиск по конкретному слову и удалнеи его до него 
...
/* 
    ...
    NSRange searchRande = NSMakeRange(0 , [text length]);
    
    NSInteger counter = 0;
    
    while (YES) {
        NSRange range = [text rangeOfString:@"NSString" options:0 range:searchRande];
        
        if (range.location != NSNotFound) {
        
            NSInteger index = range.location + range.length;  // прыгаем на одно значение вперед
            
            searchRande.location = index; // Начнем поиск с этого места
            
            searchRande.length = [text length] - index;
...
*/
    ...
    // Заменить словов на другое слово!
    //text = [text stringByReplacingOccurrencesOfString:@"NSString" withString:@"Tra-la-la"];
    
    //text = [text uppercaseString]; //Все с большеой буквы
    
    //text = [text capitalizedString]; // Каздое слово с боольшой буквы
    
    //!!!!!!!!!!!
    // Регуляры выражения почитать! в поле майла например чтоб фуфло не писпли
    //!!!!!!!!!!!!
/*
